...
def train(
    ...
    valid_sample_fixed_x, valid_sample_moved_x, valid_sample_sensitivity_map, valid_sample_single_fixed_y, valid_sample_single_fixed_mask, valid_sample_single_fixed_y_tran, valid_sample_single_moved_y, valid_sample_single_moved_mask, valid_sample_single_moved_y_tran, \
    ...
    for global_epoch in range(1, train_epoch):
        ...
        for i, train_data in enumerate(iter_):
            ...
            if is_optimize_regis:
                ...
                for j in range(regis_batch):
                    ...
                    #print("\n\n\n\n\n(1)(DeCoLearn)fixed_y_tran_recon.shape [before]: ", fixed_y_tran_recon.shape)
                    '''
                    ...
                    print("(2)(DeCoLearn)fixed_y_tran_recon.shape [before]: ", fixed_y_tran_recon.shape)
                    print("\n\n\n\n")
                    '''
            ...
            for j in range(recon_batch):
                ...
                if is_optimize_regis:
                    ...
                    #print("\n\n\n(1) YOUNGIL]fixed_y_tran_recon.shape: ", fixed_y_tran_recon.shape)
                    ...
                    import fastmri
                    fixed_y_tran_recon_res = fixed_y_tran_recon.permute([0, 2, 3, 1]).contiguous()
                    fixed_y_tran_recon_abs = fastmri.complex_abs(fixed_y_tran_recon_res)
                    fixed_y_tran_recon_abs = fixed_y_tran_recon_abs.unsqueeze(1)

                    moved_y_tran_recon_res = moved_y_tran_recon.permute([0, 2, 3, 1]).contiguous()
                    moved_y_tran_recon_abs = fastmri.complex_abs(moved_y_tran_recon_res)
                    moved_y_tran_recon_abs = moved_y_tran_recon_abs.unsqueeze(1)

                    fixed_y_tran_recon_abs = torch.nn.functional.pad(fixed_y_tran_recon_abs, [4, 4])
                    moved_y_tran_recon_abs = torch.nn.functional.pad(moved_y_tran_recon_abs, [4, 4])
                    '''

                    #print("\n\n\n(2)YOUNGIL]fixed_y_tran_recon.shape: ", fixed_y_tran_recon_res.shape)
        ...
        with torch.no_grad():
            ...
            for i, valid_data in enumerate(iter_):
                ...
                elif recon_module_type== "red" or recon_module_type=="deq":
                    gammaList = recon_module.getGamma()
            ...
            valid_sample_fixed_y_tran_recon = valid_sample_fixed_y_tran_recon.permute([0, 2, 3, 1]).contiguous()
            valid_sample_fixed_y_tran_recon = fastmri.complex_abs(valid_sample_fixed_y_tran_recon)
            valid_sample_fixed_y_tran_recon = valid_sample_fixed_y_tran_recon.unsqueeze(1)

            valid_sample_moved_y_tran_recon = valid_sample_moved_y_tran_recon.permute([0, 2, 3, 1]).contiguous()
            valid_sample_moved_y_tran_recon = fastmri.complex_abs(valid_sample_moved_y_tran_recon)
            valid_sample_moved_y_tran_recon = valid_sample_moved_y_tran_recon.unsqueeze(1)

            valid_sample_moved_y_tran_recon = torch.nn.functional.pad(valid_sample_moved_y_tran_recon, [4, 4])
            valid_sample_fixed_y_tran_recon = torch.nn.functional.pad(valid_sample_fixed_y_tran_recon, [4, 4])
            '''
